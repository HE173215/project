...
export const UserProvider = ({ children }) => {
  ...
  const isAdmin = useMemo(() => (authUser?.roles || []).includes("admin"), [authUser])
  ...
  const assignRoles = useCallback(async (userId, roles) => {
    ...
    try {
      ...
      setUsers((prev) =>
        prev.map((user) => (user.id === userId ? { ...data.user, roles: normalizeRoles(data.user.roles) } : user)),
      )
  ...
  }, [isAdmin, fetchUsers, fetchOwnProfile])
